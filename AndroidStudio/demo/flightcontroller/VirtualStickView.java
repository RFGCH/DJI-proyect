...
public class VirtualStickView extends RelativeLayout implements View.OnClickListener, CompoundButton.OnCheckedChangeListener, PresentableView {
    ...
    private void init(Context context) {
        ...
        // Inicializamos el firebase y seteamos los valores a 0
        FirebaseApp.initializeApp(getContext());
        firebaseRef = FirebaseDatabase.getInstance().getReference("0001");
        firebaseRef.child("x").setValue(0);
        firebaseRef.child("y").setValue(0);
        firebaseRef.child("z").setValue(0);
        firebaseRef.child("pitch").setValue(0);
        firebaseRef.child("roll").setValue(0);
        firebaseRef.child("yaw").setValue(0);
            ...
            simulator.setStateCallback(new SimulatorState.Callback() {
                ...
                public void onUpdate(@NonNull final SimulatorState simulatorState) {
                    ToastUtils.setResultToText(textView,
                            "Yaw : " + Math.round(simulatorState.getYaw()) + " , "
                             + "X : " + Math.round(simulatorState.getPositionX()) + "\n"

                             + "Y : " + Math.round(simulatorState.getPositionY()) + " , "
                             + "Z : " + Math.round(simulatorState.getPositionZ()) + "\n"

                             + "Right X : " + screenJoystickLeft.mKnobX + " , "
                             + "Y : " + screenJoystickLeft.mKnobY + "\n"

                             + "Left X : " + screenJoystickRight.mKnobX + " , "
                             + "Y : " + screenJoystickRight.mKnobY + "\n"
        ...
        firebaseRef.addValueEventListener(new ValueEventListener() {
            ...
            public void onDataChange(@NonNull DataSnapshot dataSnapshot) {
                ...
                if (dataSnapshot.exists()) {
                    ...
                    Long pitchValue = dataSnapshot.child("pitch").getValue(Long.class);
                    Long rollValue = dataSnapshot.child("roll").getValue(Long.class);
                    Long yawValue = dataSnapshot.child("yaw").getValue(Long.class);
                    if (pitchValue != null && rollValue != null && yawValue != null) {
                        // Convert received pitch, roll, and yaw values to radians (if necessary)
                        float pitchFloat = Math.round(pitchValue); // Convert to float with 1 decimal place
                        float rollFloat = Math.round(rollValue);    // Convert to float with 1 decimal place
                        float yawFloat = Math.round(yawValue );      // Convert to float with 1 decimal place

// Display the converted float values using Toast
                        //Toast.makeText(getContext(), "Pitch: " + pitchFloat, Toast.LENGTH_SHORT).show();
                        //Toast.makeText(getContext(), "Roll: " + rollFloat, Toast.LENGTH_SHORT).show();
                        //Toast.makeText(getContext(), "Yaw: " + yawFloat, Toast.LENGTH_SHORT).show();


                        // Rotate the gimbal based on the received pitch, roll, and yaw angles
                        DJISampleApplication.getProductInstance().getGimbal().rotate(
                                new Rotation.Builder()
                                        .pitch(pitchFloat)
                                        .roll(rollFloat)
                                        .yaw(yawFloat)
                                        .mode(RotationMode.ABSOLUTE_ANGLE)
                                        .time(0)  // Adjust the time as needed (in seconds)
                                        .build(),
                                new CommonCallbacks.CompletionCallback() {
                                    @Override
                                    public void onResult(DJIError error) {
                                        if (error == null) {
                                            // Gimbal rotation successful
                                            Toast.makeText(getContext(), "success rotation", Toast.LENGTH_SHORT).show();

                                        } else {
                                            // Handle gimbal rotation error
                                            Toast.makeText(getContext(), "error rotation", Toast.LENGTH_SHORT).show();
                    ...
                    if(xValue.toString().equals("1")) {
                        ToastUtils.setResultToToast("Adelante");
                        roll = 100;
                        sendVirtualStickDataTask = new SendVirtualStickDataTask();
                        sendVirtualStickDataTimer = new Timer();
                        sendVirtualStickDataTimer.schedule(sendVirtualStickDataTask, 100, 200);
    ...
    private void tearDownListeners() {
        ...
        firebaseRef.addValueEventListener(null);
    }
